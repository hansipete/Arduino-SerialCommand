...
void SerialCommand::addCommand(const char *command, void (*function)()) {
    ...
    SerialPort.print("Adding command (");
...
void SerialCommand::readSerial() {
  while (SerialPort.available() > 0) {
    char inChar = SerialPort.read();   // Read single available character, there may be more waiting
    #ifdef SERIALCOMMAND_DEBUG
      SerialPort.print(inChar);   // Echo back to serial stream
    #endif
    ...
    if (inChar == term) {     // Check for the terminator (default '\r') meaning end of command
      ...
      if (bufPos < SERIALCOMMAND_BUFFER) {
        ...
        #ifdef SERIALCOMMAND_DEBUG
          SerialPort.println("Line buffer is full - increase SERIALCOMMAND_BUFFER");
        #endif
      }
    }
  }
